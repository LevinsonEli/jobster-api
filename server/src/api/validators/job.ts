...
// import { IGetAllJobsValidatedInput } from '../../interfaces/user-inputs/IJobs';

// const validateGetAllJobsInput = (input: any): IGetAllJobsValidatedInput => {
//   const search =
//     input.search?.replace(/\.|\+|\*|\?|\^|\$|\(|\)|\[|\]|\{|\}|\||\\/gi, '') ||
//     '';
//   const status =
...
//       : input.status;
//   const type =
//     input.type && input.type !== 'all' ? validateType(input.type) : input.type;
//   const sort = validateSort(input.sort);
//   const page = Number(input.page) || 1;
//   const limit = Number(input.limit) || 10;
//   const skip = (page - 1) * limit;

//   return { search, status, type, sort, page, limit, skip } as IGetAllJobsValidatedInput;
// };
...
// JobValidator.getValidatedFor('getAllJobs')({ ...req.query, userId: req.user.id });
...
import { IGetAllJobsInputDTO, ICreateJobInputDTO, IUpdateJobInputDTO } from '../../interfaces/user-inputs/IJobs';
...
export default class JobValidator {
  ...
  public validateGetAllJobsInput = (input: IGetAllJobsInputDTO): GetAllJobsValidatedInput => {
    const search =
      input.search?.replace(/\.|\+|\*|\?|\^|\$|\(|\)|\[|\]|\{|\}|\||\\/gi, '') ||
      '';
    const status =
      input.status && input.status !== 'all'
        ...
        : input.status;
    const type =
      input.type && input.type !== 'all' ? this.validateType(input.type) : input.type;
    const sort = this.validateSort(input.sort);
    const page = Number(input.page) || 1;
    const limit = Number(input.limit) || 10;
    const skip = (page - 1) * limit;

    return { search, status, type, sort, page, limit, skip } as GetAllJobsValidatedInput;
  };

  public validateCreateJobInput = (input: ICreateJobInputDTO): CreateJobValidatedInput => {
      ...
      if ( !validateId ( createdBy ) )
          throw new UnauthenticatedError('Must login first');
      const location = input.location ? this.validateLocation(input.location) : undefined;
      const type = input.type ? this.validateType(input.type) : undefined;

      return { company, position, status, createdBy, location, type } as CreateJobValidatedInput;
  }

  public validateUpdateJobInput = (input: IUpdateJobInputDTO): UpdateJobValidatedInput => {
    const company = this.validateCompany(input.company);
    const position = this.validatePosition(input.position);
    const status = input.status ? this.validateStatus(input.status) : undefined;
    if (!validateId(input.userId))
    ...
    return { company, position, status, location, type } as UpdateJobValidatedInput;
  };
    
  public validateCompany = ( company: string ) => {
      if ( !company )
          throw new BadRequestError('Company name is required for Job');
      if ( company.length > 50 )
          throw new BadRequestError(`Company name can me the most ${50} characters`);
  ...
  public validateLocation = ( location: string ) => {
      if ( location.length > 30 )
          throw new BadRequestError(`Location can be the most ${30} characters long`);
      return location;
  }
  ...
  public validateSort = ( sort: string ) => {
      const sortsList = ['a-z', 'z-a', 'latest', 'oldest'];
      const defaultSortValue = sortsList[0];
